...
select is((select count(*) from public.cards where learner_id = auth.uid()), 4::bigint, 'legacy Cards exist before unscheduled Review');
...
select set_config(
  'test.question_id',
  (select id::text from public.review_session_questions where session_id = current_setting('test.session_id')::uuid),
  true
);
select set_config(
...
select is((select count(*) from public.review_session_questions where session_id = current_setting('test.session_id')::uuid), 1::bigint, 'one deterministic question is persisted');
...
select is(jsonb_array_length(public.review_session_overview(current_setting('test.learning_language_id')::uuid)->'questions'), 1, 'overview returns one question');
select is((public.review_session_overview(current_setting('test.learning_language_id')::uuid)->'questions'->0->>'direction'), 'recognition', 'the minimal question uses recognition');
...
select lives_ok(
  ...
  'the first answer completes the session'
);
...
select is((select status from public.review_sessions where id = current_setting('test.session_id')::uuid), 'completed', 'the completed status is durable');
select is((select correct_count from public.review_sessions where id = current_setting('test.session_id')::uuid), 1, 'the completed correctness result is durable');
...
select is((select direction from public.review_attempts where session_id = current_setting('test.session_id')::uuid), 'recognition', 'the Attempt snapshots its direction');
select is((public.review_session_overview(current_setting('test.learning_language_id')::uuid)->'questions'->0->>'correct_answer'), current_setting('test.correct_answer'), 'completed overview exposes the persisted correct answer');
select is((public.review_session_overview(current_setting('test.learning_language_id')::uuid)->'questions'->0->>'answered_at'), current_setting('test.answered_at'), 'completed overview preserves the first answer timestamp');
...
select is((select count(*) from public.review_sessions where id = '33330000-0000-0000-0000-000000000011' and status = 'completed' and completed_at is not null and correct_count = 1), 1::bigint, 'concurrent submissions complete the session once');
...
select is((select count(*) from public.review_attempts where session_id = current_setting('test.session_id')::uuid), 1::bigint, 'suspension preserves recorded Attempts');
set local role postgres;
...
set local role authenticated;
select is((select count(*) from public.review_attempts where session_id = current_setting('test.session_id')::uuid), 0::bigint, 'deleting a Vocabulary Entry removes its Attempt details');
