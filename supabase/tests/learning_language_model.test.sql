...
values (
  '79797979-7979-7979-7979-797979797979',
  (select sense_id from public.translations where text = 'house'),
  'будинок',
  'uk'
);
insert into public.translations (learner_id, sense_id, text, answer_language_tag)
values (
  '79797979-7979-7979-7979-797979797979',
  (select sense_id from public.translations where text = 'дім'),
  'home',
  'en'
);
...
values (
  '79797979-7979-7979-7979-797979797979',
  (select id from public.learning_languages where language_tag = 'es'),
  'Homes'
);
...
select is((select answer_language_tag from public.preferred_answer_languages), 'en', 'a deterministic recent-use tie selects the Preferred Answer Language');
...
select public.set_vocabulary_entry_suspended((select id from public.vocabulary_entries limit 1), true);
select is((select suspended from public.learning_vocabulary_entries), true, 'canonical suspension follows compatibility writes');
...
select public.set_vocabulary_entry_suspended((select id from public.vocabulary_entries where suspended limit 1), false);
...
select is(
  (select count(*) from public.learning_scheduled_review_overview(
    (select id from public.learning_languages where language_tag = 'fr')
  )),
  0::bigint,
...
select set_config(
  ...
  (select cards.id::text
   ...
   where translations.text = 'house' and cards.answer_language_tag = 'en' and cards.direction = 'recognition'),
  true
);
select lives_ok(
  format(
    $$select public.move_vocabulary_entries(array[%L::uuid], %L::uuid)$$,
    (select vocabulary_entries.id from public.vocabulary_entries join public.study_pairs on study_pairs.id = vocabulary_entries.study_pair_id where study_pairs.target_language_tag = 'es' and study_pairs.reference_language_tag = 'en'),
...
select is(
  (select learning_languages.language_tag
   ...
   where cards.id = current_setting('test.moved_card_id')::uuid),
  'fr',
  'moved Cards follow the new Learning Language without changing identity'
);

select lives_ok(
  $$select public.set_active_learning_language((select id from public.learning_languages where language_tag = 'fr'))$$,
  'the Learner can synchronize another Active Learning Language'
);
select is(
  (select language_tag from public.learning_languages join public.learner_language_state on active_learning_language_id = learning_languages.id),
  'fr',
...
select throws_ok(
  $$select public.set_active_learning_language((select id from public.learning_languages limit 1))$$,
  'P0001',
