...
export function App() {
  ...
  return <main className="shell command-center" data-ui="extension-popup"><header className="popup-header" data-ui="popup-header"><div className="popup-identity"><Brand compact /><div><strong>Lexync</strong><span>Reading companion</span></div></div><p className="identity" data-ui="profile-account">{session.user.email}</p></header><div className="workspace-label"><span>Today</span><strong>Keep your reading loop moving</strong></div><section className="task-context task-first-panel" data-ui="task-first-panel"><div className="context-copy"><span className="section-label">Learning language</span><p>Used for capture and page highlights</p></div><div className="select-field" data-ui="language-switcher"><label htmlFor="active-learning-language">Active Learning Language</label><div className="select-wrap"><select id="active-learning-language" aria-label="Active Learning Language" disabled={submitting} value={activeLanguageId} onChange={(event) => void selectActiveLanguage(event.target.value)}>{languages.map((language) => <option key={language.id} value={language.id}>{languageName(language.languageTag)}</option>)}</select><span className="select-indicator" data-ui="select-indicator" aria-hidden="true">⌄</span></div></div>{languages.length === 0 && <p className="panel-note">Add a learning language in your Lexync library to start capturing words.</p>}</section><section className="mode-status-region task-first-panel" data-ui="task-first-panel"><div className="mode-heading"><div><span className="section-label">Current page</span><h2>{learningSite?.origin ? new URL(learningSite.origin).hostname : 'Ready when you are'}</h2></div><span className={`page-state ${learningSite?.enabled ? 'enabled' : ''}`} role="status">{learningSite?.enabled ? 'Learning Mode on' : 'Waiting for a page'}</span></div>{learningSite ? <><p>{learningSite.enabled ? 'Learning Mode is active. Saved expressions are highlighted as you read.' : learningSite.permitted ? 'Site access is ready. Return to the page to enable Learning Mode.' : `Lexync found ${detectedLanguage ? languageName(detectedLanguage) : 'a language'} on this site.`}</p>{learningSite.enabled ? <button className="secondary" type="button" onClick={() => void setLearningMode(false)}>Disable Learning Mode</button> : !learningSite.permitted ? <button className="secondary" type="button" onClick={() => void setUpLearningMode()}>Set up Learning Mode</button> : learningSite.selectedLearningLanguageId ? <button className="secondary" type="button" onClick={() => void setLearningMode(true)}>Enable Learning Mode</button> : null}</> : <p className="panel-note" data-ui="empty-state">Open a reading page and Lexync will bring its language tools here.</p>}</section><section className="capture-workspace task-first-panel" data-ui="task-first-panel"><div className="capture-heading"><div><span className="section-label">Capture</span><h2>Capture a word</h2></div><p>Private to your library</p></div>{languages.length > 0 ? <form onSubmit={saveVocabularyEntry} noValidate><div className="capture-fields"><label>Expression<input value={expression} onChange={(event) => setExpression(event.target.value)} placeholder="incontro" />{errors.expression && <span className="field-error">{errors.expression}</span>}</label><label>Translation<input value={translation} onChange={(event) => setTranslation(event.target.value)} placeholder="meeting" />{errors.translation && <span className="field-error">{errors.translation}</span>}</label></div><div className="capture-fields"><label>Answer Language<input value={answerLanguage} onChange={(event) => setAnswerLanguage(event.target.value)} placeholder="en" />{errors.answerLanguage && <span className="field-error">{errors.answerLanguage}</span>}</label><label>Example <span className="optional">Optional</span><textarea value={example} onChange={(event) => setExample(event.target.value)} rows={2} placeholder="A sentence worth keeping" /></label></div>{senseDecision && <fieldset className="sense-choice"><legend>Choose a Sense</legend>{senseDecision.response.senses.map((sense) => <label key={sense.id}><input checked={senseDecision.selected === sense.id} name="sense" type="radio" onChange={() => setSenseDecision({ ...senseDecision, selected: sense.id })} />{sense.translations.map((item) => `${item.text} (${item.answerLanguageTag})`).join(' · ') || 'Existing Sense'}</label>)}<label><input checked={senseDecision.createNew} name="sense" type="radio" onChange={() => setSenseDecision({ ...senseDecision, selected: undefined, createNew: true })} />Create a new Sense</label></fieldset>}<button className="primary" disabled={submitting || Boolean(senseDecision && !senseDecision.selected && !senseDecision.createNew)} type="submit">Save Vocabulary Entry</button></form> : <div className="capture-empty"><p className="panel-note">Your quick capture form will appear after you add a learning language.</p><a className="secondary-link" href={`${import.meta.env.WXT_PUBLIC_WEB_URL}/settings`} rel="noreferrer" target="_blank">Add a Learning Language</a></div>}</section>{notice && <p className={`notice ${noticeTone}`} role={noticeTone === 'error' ? 'alert' : 'status'}>{notice}</p>}</main>;
}
