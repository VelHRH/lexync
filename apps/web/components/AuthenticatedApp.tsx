...
export function AuthenticatedApp({ section = 'Home', publicContent, forceOnboarding = false }: { section?: string; publicContent?: ReactNode; forceOnboarding?: boolean }) {
  ...
  useEffect(() => {
    if (!session) return;
    void supabase.from('study_pairs').select('id,is_primary,target_language_tag,reference_language_tag').order('created_at').then(({ data }) => {
      ...
      setPairs(nextPairs);
      setActivePairId((current) => current || nextPairs[0]?.id || '');
    ...
    <main className="app-shell">
      ...
      <div className="app-body">
        ...
        <section className="app-content" aria-labelledby="app-heading">
          ...
          <select id="active-study-pair" aria-label="Active Study Pair" value={activePairId} onChange={(event) => setActivePairId(event.target.value)}>{pairs.map((pair) => <option key={pair.id} value={pair.id}>{studyPairLabel(pair)}</option>)}</select>
          <div className="pair-management"><button className="secondary-button" type="button" onClick={() => setShowPairForm((current) => !current)}>Add Study Pair</button>{pairs.map((pair) => <div className="pair-row" key={pair.id}><span>{studyPairLabel(pair)} {pair.isPrimary && <strong>Primary</strong>}</span><div><button className="text-button" type="button" onClick={() => void setPrimary(pair.id)} disabled={pair.isPrimary}>Make primary {studyPairLabel(pair)}</button><button className="text-button danger" type="button" onClick={() => void deletePair(pair.id)} disabled={pairs.length === 1}>Delete {studyPairLabel(pair)}</button></div></div>)}</div>
          {showPairForm && <StudyPairOnboarding completeImmediately onCreated={(pair) => { setPairs((current) => [...current, pair]); setActivePairId(pair.id); setShowPairForm(false); }} />}
        </section>
      </div>
    </main>
  );
